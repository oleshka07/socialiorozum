...
Для скрінкаста потрібні **реальні** сторінка+IG (Meta-«Test Users» не можуть мати справжній IG Business). Окремий особистий FB-акаунт НЕ створюй — це проти правил Meta (одна людина = один акаунт) і ризик бана. Достатньо:
- **Твій наявний особистий FB** — ти адмін застосунку, тож у Development mode все працює для тебе без додавання в Testers (Testers — лише для сторонніх людей).
- **Нова demo FB-сторінка** під твоїм акаунтом (сторінок можна мати скільки завгодно).
- **Новий Instagram** → перемкнути на Business/Creator → прив'язати до demo-сторінки → 3-5 постів (щоб «Голос з Instagram» мав що читати). Особистий (не-business) IG наш API не побачить.
- ⚠️ Креденшели від FACEBOOK рецензенту НЕ передаються (заборонено правилами) — тільки email+пароль від socialio (КРОК 2).
