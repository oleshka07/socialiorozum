...
## Разове розгортання — АВТОМАТИЧНЕ
1. DNS: A-запис `beta.socialio.rozum.one` → `178.105.185.67` (зроблено).
2. GitHub → Actions → **provision-beta** → Run workflow. Він сам: клонує `/opt/socialio-beta`, збере `.env` бети (копія прод + бета-рядки, новий пароль БД генерується на сервері), підніме docker-стек, пропише nginx і отримає SSL-сертифікат. Ідемпотентний — можна перезапускати.
3. Якщо крок SSL упав «DNS ще не розійшовся» — перезапусти workflow за 10-30 хв.

Нижче — ті самі кроки ВРУЧНУ (запасний варіант, якщо Actions чомусь не може):
